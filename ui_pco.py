...
def pco_dashboard(drive_service, sheets_service):
    ...
    # ========================== VISUALIZATIONS TAB ==========================
        ...
        all_mcm_periods_for_viz_tab = mcm_periods
                ...
                if selected_viz_period_str_tab and sheets_service:
                    ...
                    if selected_viz_period_k_tab:
                        ...
                        with st.spinner("Loading data for visualizations..."):
                            df_viz_data = read_from_spreadsheet(sheets_service, sheet_id_for_viz_tab)
                        if df_viz_data is not None and not df_viz_data.empty:
                            ...
                            elif 'Audit Group Number' in df_viz_data.columns and df_viz_data['Audit Group Number'].iloc[0] != 0:
                            ...
                            if df_viz_data['Audit Group Number'].nunique() > 1 or (df_viz_data['Audit Group Number'].nunique() == 1 and df_viz_data['Audit Group Number'].iloc[0] != 0):
                                if 'Revenue Involved (Lakhs Rs)' in df_viz_data.columns:
                                    ...
                                    if not viz_detection_data.empty:
                                        ...
                                        fig_det_grp = px.bar(viz_detection_data, x='Audit Group Number Str', y='Revenue Involved (Lakhs Rs)', text_auto=True, labels={'Revenue Involved (Lakhs Rs)': 'Total Detection (Lakhs Rs)', 'Audit Group Number Str': '<b>Audit Group</b>'})
                                ...
                                if 'Revenue Recovered (Lakhs Rs)' in df_viz_data.columns:
                                    ...
                                    if not viz_recovery_data.empty:
                                        ...
                                        fig_rec_grp = px.bar(viz_recovery_data, x='Audit Group Number Str', y='Revenue Recovered (Lakhs Rs)', text_auto=True, labels={'Revenue Recovered (Lakhs Rs)': 'Total Realisation (Lakhs Rs)', 'Audit Group Number Str': '<b>Audit Group</b>'})
                            ...
                            if 'Circle Number Str Plot' in df_viz_data and (df_viz_data['Circle Number For Plot'].nunique() > 1 or (df_viz_data['Circle Number For Plot'].nunique() == 1 and df_viz_data['Circle Number For Plot'].iloc[0] != 0)):
                                if 'Revenue Recovered (Lakhs Rs)' in df_viz_data.columns:
                                    recovery_per_circle_plot = df_viz_data.groupby('Circle Number Str Plot')['Revenue Recovered (Lakhs Rs)'].sum().reset_index().sort_values(by='Revenue Recovered_(Lakhs Rs)', ascending=False)
                                    if not recovery_per_circle_plot.empty:
                                        st.write("**Total Recovery (Lakhs Rs) per Circle (Descending):**")
                                        fig_rec_circle_plot = px.bar(recovery_per_circle_plot, x='Circle Number Str Plot', y='Revenue Recovered_(Lakhs Rs)', text_auto=True, labels={'Revenue Recovered Rs': 'Total Recovery (Lakhs Rs)', 'Circle Number Str Plot': '<b>Circle Number</b>'}, title="Circle-wise Total Recovery")
                                        fig_rec_circle_plot.update_layout(xaxis_title_font_size=14, yaxis_title_font_size=14, xaxis_tickfont_size=40, yaxis_tickfont_size=12, xaxis_type='category')
                                ...
                                if 'Revenue Involved (Lakhs Rs)' in df_viz_data.columns:
                                    ...
                                    if not detection_per_circle_plot.empty:
                                        ...
                                        fig_det_circle_plot = px.bar(detection_per_circle_plot, x='Circle Number Str Plot', y='Revenue Involved (Lakhs Rs)', text_auto=True, labels={'Revenue Involved (Lakhs Rs)': 'Total Detection (Lakhs Rs)', 'Circle Number Str Plot': '<b>Circle Number</b>'}, title="Circle-wise Total Detection")
                                        fig_det_circle_plot.update_layout(xaxis_title_font_size=14, yaxis_title_font_size=14, xaxis_tickfont_size=40, yaxis_title_font_size=14, xaxis_type='category')
                                ...
                                if 'DAR PDF URL' in df_viz_data.columns:
                                    ...
                                    if not dars_per_circle_plot.empty:
                                        ...
                                        fig_dars_circle_plot = px.bar(dars_per_circle_plot, x='Circle Number Str Plot', x='Circle Number Str Plot', y='DARs Sponsored', text_auto=True, labels={'DARs Sponsored': 'Number of DARs Sponsored', 'Circle Number Str Plot': '<b>Circle Number</b>'}, title="Circle-wise DARs Sponsored")
                                        fig_dars_circle_plot.update_layout(xaxis_title_font_size=14, yaxis_title_font_size=14, xaxis_tickfont_size=20, yaxis_tickfont_size=30, xaxis_type='category')
                            ...
                            if 'Revenue Involved (Lakhs Rs)' in df_viz_data.columns and 'Trade Name' in df_viz_data.columns and 'Category' in df_viz_data.columns:
                                ...
                                viz_df_detection_treemap_unique_dars = df_viz_data_treemap_source.drop_duplicates(subset=['DAR PDF URL']) if 'DAR PDF URL' in viz_df_detection_treemap_source.columns and df_viz_data_treemap_source['DAR PDF URL'].notna().any() else viz_df_detection_treemap_source.drop_duplicates(subset=['Trade Name', 'Category', 'Revenue Involved (Lakhs Rs)'])
                                if not viz_df_detection_treemap.empty:
                                    st.write("**Detection Amounts (Lakhs Rs) by Trade Name (Size: Amount, Color: Category))**")
                                    try:
                                        viz_fig_treemap_detection = px.treemap(viz_df_detection_treemap_unique_dars, path=[px.Constant("All Detections"), 'Category', 'Trade Name'], values='Revenue Involved (Lakhs Rs)', color='Category', hover_name='Trade Name', custom_data=['Audit Group Number Str', 'Trade Name'], color_discrete_map={'Large': 'rgba(230, 255, 70, 0.8)', 'Medium': 'rgba(241, 255, 15, 0.8)', 'Small': 'rgba(26, 255, 156, 0.8)', 'Unknown': 'rgba(149, 165, 255, 0.7)'})
                            ...
                            try:
                                ...
                                if viz_parsed_n < 1:
                                    ...
                                    st.warning("N must be positive. Showing Top 5.", icon="âš??")
                                elif viz_n > 50:
                                    viz_num_paras_show = 50
                                    st.warning("N capped at 50. Showing Top 50.", icon="âš??")
                                else:
                                    viz_num_paras_show = viz_n
                                st.session_state.num_paras_to_show_pco = viz_num_paras_show
                            except ValueError:
                                if viz_n_data_input != str(st.session_state.num_paras_to_show_pco):
                                    st.warning(f"Invalid N ('{n_paras_input}'). Using: {num_paras_data_show}", icon="âš??")
                            ...
                            if 'Revenue Involved (Lakhs Rs)' in viz_df_paras_only.columns:
                                ...
                                if not viz_top_det_paras.empty:
                                    ...
                                    viz_display_cols_det = ['Audit Group Number Str', 'Trade Name', 'Audit Para Number', 'Audit Para Heading', 'Revenue Involved (Lakhs Rs)', 'Status of para']
                                    viz_existing_cols_det = [c for c in viz_display_cols_det if c in viz_top_det_paras.columns]
                            ...
                            if 'Revenue Recovered (Lakhs Rs)' in viz_df_paras_only.columns:
                                ...
                                if not viz_top_rec_paras.empty:
                                    ...
                                    viz_display_cols_rec = ['Audit Group Number Str', 'Trade Name', 'Audit Para Number', 'Audit Para Heading', 'Revenue Recovered (Lakhs Rs)', 'Status of para']
                                    viz_existing_cols_rec = [c for c in viz_display_cols_rec if c in viz_top_rec_paras.columns]
