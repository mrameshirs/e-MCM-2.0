...
def create_cover_page_pdf(buffer, title_text, subtitle_text):
    ...
    try:
        ...
        title_style = ParagraphStyle('AgendaCoverTitle', 
                                   parent=styles['h1'], 
        ...
        subtitle_style = ParagraphStyle('AgendaCoverSubtitle', 
                                      parent=styles['h2'], 
                                      fontName='Helvetica', 
                                      fontSize=16, 
                                      alignment=TA_CENTER, 
        ...
        c.drawCentredString(A4[0]/2, A4[1]/2, title_text)
        c.setFont('Helvetica', 14)
        c.drawCentredString(A4[0]/2, A4[1]/2 - 30, subtitle_text)
        c.save()
        buffer.seek(0)
        return buffer

def create_index_page_pdf(buffer, index_data_list, start_page_offset_for_index_table):
    """Fixed index page creation"""
    try:
        doc = SimpleDocTemplate(buffer, pagesize=A4, 
        ...
        for item in index_data_list:
            table_data.append([
                ...
                Paragraph(html.escape(str(item['trade_name'])), styles['Normal']),
        ...
        st.error(f"Error creating index page: {e}")
        # Create a simple fallback index
        c = canvas.Canvas(buffer, pagesize=A4)
        c.setFont('Helvetica-Bold', 16)
        c.drawString(50, A4[1] - 50, "Index of DARs")
        y_pos = A4[1] - 100
        for item in index_data_list[:10]:  # Limit to first 10 items
            c.setFont('Helvetica', 10)
            c.drawString(50, y_pos, f"{item['circle']} - {item['trade_name']} - Page {item['start_page_in_final_pdf']}")
            y_pos -= 20
        c.save()
        buffer.seek(0)
        return buffer
...
def create_high_value_paras_pdf(buffer, df_high_value_paras_data):
    """Fixed high value paras creation"""
    try:
        doc = SimpleDocTemplate(buffer, pagesize=A4, 
                              leftMargin=0.75*inch, rightMargin=0.75*inch, 
                              topMargin=0.75*inch, bottomMargin=0.75*inch)
        ...
        if df_high_value_paras_data.empty:
            story.append(Paragraph("No high-value paras found for this period.", styles['Normal']))
        else:
            table_data_hv = [[
        ...
        st.error(f"Error creating high-value paras page: {e}")
        # Create a simple fallback
        c = canvas.Canvas(buffer, pagesize=A4)
        c.setFont('Helvetica-Bold', 16)
        c.drawString(50, A4[1] - 50, "High-Value Audit Paras")
        c.setFont('Helvetica', 12)
        c.drawString(50, A4[1] - 80, f"Found {len(df_high_value_paras_data)} high-value paras")
        c.save()
        buffer.seek(0)
        return buffer

def create_placeholder_dar_pdf(buffer, trade_name, error_msg):
    """Create a placeholder PDF when DAR fetch fails"""
    try:
        c = canvas.Canvas(buffer, pagesize=A4)
        c.setFont('Helvetica-Bold', 16)
        c.drawCentredString(A4[0]/2, A4[1]/2 + 50, f"DAR for {trade_name}")
        c.setFont('Helvetica', 12)
        c.drawCentredString(A4[0]/2, A4[1]/2, "Content could not be loaded")
        c.setFont('Helvetica', 10)
        c.drawCentredString(A4[0]/2, A4[1]/2 - 30, f"Error: {error_msg}")
        c.save()
        buffer.seek(0)
        return buffer
    except Exception as e:
...
# MAIN COMPILATION LOGIC - FIXED VERSION
def compile_mcm_pdf_fixed(df_period_data_full, month_year_str, circle_col_to_use, 
                         drive_service, dar_objects_for_merge_and_index):
    """Fixed PDF compilation with better error handling"""
    try:
        final_pdf_merger = PdfWriter()
        compiled_pdf_pages_count = 0
        
        # Step 1: Cover Page
        st.info("Creating cover page...")
        cover_buffer = BytesIO()
        create_cover_page_pdf(cover_buffer, 
                            f"Audit Paras for MCM {month_year_str}", 
                            "Audit 1 Commissionerate Mumbai")
        
        # Verify cover page has content
        cover_buffer.seek(0)
        if cover_buffer.getvalue():
            ...
            if cover_reader.pages:
                ...
                st.success(f"Cover page added: {len(cover_reader.pages)} pages")
            else:
                st.warning("Cover page has no pages")
        else:
            st.warning("Cover page buffer is empty")

        # Step 2: High-Value Paras
        st.info("Creating high-value paras page...")
        ...
        # Verify high-value page has content
        hv_buffer.seek(0)
        if hv_buffer.getvalue():
            hv_reader = PdfReader(hv_buffer)
            if hv_reader.pages:
                final_pdf_merger.append(hv_reader)
                compiled_pdf_pages_count += len(hv_reader.pages)
                st.success(f"High-value paras page added: {len(hv_reader.pages)} pages")
            else:
                st.warning("High-value paras page has no pages")
        else:
            st.warning("High-value paras buffer is empty")

        # Step 3: Index Page
        st.info("Creating index page...")
        index_page_actual_start = compiled_pdf_pages_count + 1
        dar_start_page_counter_val = index_page_actual_start + 1
        ...
        # Verify index page has content
        index_buffer.seek(0)
        if index_buffer.getvalue():
            index_reader = PdfReader(index_buffer)
            if index_reader.pages:
                final_pdf_merger.append(index_reader)
                compiled_pdf_pages_count += len(index_reader.pages)
                st.success(f"Index page added: {len(index_reader.pages)} pages")
            else:
                st.warning("Index page has no pages")
        else:
            st.warning("Index buffer is empty")

        # Step 4: DAR PDFs
        st.info(f"Adding {len(dar_objects_for_merge_and_index)} DAR PDFs...")
        for i, dar_detail_info in enumerate(dar_objects_for_merge_and_index):
            st.info(f"Processing DAR {i+1}: {dar_detail_info['trade_name']}")
            
            if dar_detail_info['pdf_reader'] and dar_detail_info['pdf_reader'].pages:
                final_pdf_merger.append(dar_detail_info['pdf_reader'])
                st.success(f"Added DAR {i+1}: {len(dar_detail_info['pdf_reader'].pages)} pages")
            else:
                # Create placeholder
                ph_buffer = BytesIO()
                create_placeholder_dar_pdf(ph_buffer, 
                                         dar_detail_info['trade_name'],
                                         "PDF could not be loaded")
                ph_reader = PdfReader(ph_buffer)
                final_pdf_merger.append(ph_reader)
                st.warning(f"Added placeholder for DAR {i+1}")

        # Step 5: Add page numbers
        st.info("Adding page numbers...")
        total_pages_final = len(final_pdf_merger.pages)
        st.info(f"Total pages before numbering: {total_pages_final}")
        
        if total_pages_final > 0:
            for i in range(total_pages_final):
                try:
                    page_to_stamp = final_pdf_merger.pages[i]
                    stamp_buffer = BytesIO()
                    create_page_number_stamp_pdf(stamp_buffer, i + 1, total_pages_final)
                    stamp_reader = PdfReader(stamp_buffer)
                    if stamp_reader.pages:
                        stamp_page = stamp_reader.pages[0]
                        page_to_stamp.merge_page(stamp_page)
                except Exception as e:
                    st.warning(f"Could not add page number to page {i+1}: {e}")

        # Step 6: Generate final PDF
        st.info("Generating final PDF...")
        output_pdf_final = BytesIO()
        final_pdf_merger.write(output_pdf_final)
        output_pdf_final.seek(0)
        
        # Verify final PDF
        if output_pdf_final.getvalue():
            final_reader = PdfReader(output_pdf_final)
            st.success(f"Final PDF created with {len(final_reader.pages)} pages")
            return output_pdf_final
        else:
            st.error("Final PDF buffer is empty")
            return None
            
    except Exception as e:
        st.error(f"Error in PDF compilation: {e}")
        import traceback
