...
def mcm_agenda_tab(drive_service, sheets_service, mcm_periods):
    ...
    # --- Improved Data Loading using Session State ---
    ...
    for circle_num_iter in range(1, 11):
        ...
        with st.expander(f"View Details for {html.escape(circle_label_iter)}", expanded=False):
            ...
            # ... (Logic for groups and tabs remains the same)
            group_labels_list = []
            group_dfs_list = []
            min_grp = (circle_num_iter - 1) * 3 + 1
            max_grp = circle_num_iter * 3
                ...
                with group_tab_widget_item:
                    ...
                    for tn_idx_iter, trade_name_item in enumerate(unique_trade_names_list):
                        ...
                        if st.session_state.get(session_key_selected_trade) == trade_name_item:
                            ...
                            # --- OLD TABLE STRUCTURE (PRESERVED AS REQUESTED) ---
                            html_rows = ""
                            total_para_det_rs = 0
                            total_para_rec_rs = 0
                            for _, para_item_row in df_trade_paras_item.iterrows():
                                ...
                                html_rows += f"""
                                <tr>
                                    <td>{p_num_str}</td><td>{p_title}</td>
                                    <td class='amount-col'>{format_inr(det_rs)}</td><td class='amount-col'>{format_inr(rec_rs)}</td>
                                    <td>{p_status}</td>
                                </tr>"""
                            total_row_html = f"""
                            <tfoot>
                                <tr style='font-weight:bold; background-color:#e9ecef;'>
                                    <td colspan="2" style="text-align:right;">Total of Paras</td>
                                    <td class='amount-col'>{format_inr(total_para_det_rs)}</td><td class='amount-col'>{format_inr(total_para_rec_rs)}</td>
                                    <td></td>
                                </tr>
                            </tfoot>"""
                            table_full_html = f"""
                            ...
                            <table class='paras-table'>
                                <thead><tr><th>Para No.</th><th>Para Title</th><th>Detection (₹)</th><th>Recovery (₹)</th><th>Status</th></tr></thead>
                                <tbody>{html_rows}</tbody>{total_row_html}
                            </table>"""
                            st.markdown(table_full_html, unsafe_allow_html=True)
                            
                            # Overall Totals (PRESERVED AS REQUESTED)
                            ...
                            st.markdown("---")
                            st.subheader("Make/Update MCM Decisions")
                            ...
                            for index, row in df_trade_paras_item.iterrows():
                                ...
                                para_title = row.get("Audit Para Heading", "N/A")
                                ...
                                decision_key = f"mcm_decision_{trade_name_item}_{p_num_str}_{index}"
                                
                                d_cols = st.columns([1, 4])
                                d_cols[0].write(f"**Para {para_num_str}:**")
                                d_cols[1].selectbox(f"Decision for Para {para_num_str}", options=decision_options, index=default_index, key=decision_key, label_visibility="collapsed")
                            
                            st.markdown("<br>", unsafe_allow_html=True) 
                            
                            # Save button logic
                            if st.button("Save Decisions", key=f"save_decisions_{trade_name_item}"):
