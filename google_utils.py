...
# def append_to_spreadsheet(sheets_service, spreadsheet_id, values_to_append):
#     # Assuming values_to_append is a list of lists, where each inner list is a row.
#     # The 'Audit Circle Number' should be pre-calculated and included in values_to_append.
#     try:
#         body = {'values': values_to_append}
#         sheet_metadata = sheets_service.spreadsheets().get(spreadsheetId=spreadsheet_id).execute()
#         sheets = sheet_metadata.get('sheets', '')
...
#         range_to_check = f"{first_sheet_title}!A1:M1" # Extended to M for the new columns
...
#             # Ensure the order matches how `values_to_append` is structured
#             header_values_list = [[
#                 "Audit Group Number", "Audit Circle Number", "GSTIN", "Trade Name", "Category",
#                 "Total Amount Detected (Overall Rs)", "Total Amount Recovered (Overall Rs)",
#                 "Audit Para Number", "Audit Para Heading",
...
#                 "DAR PDF URL", "Record Created Date"
#             ]]
#             # Determine the correct order for values_to_append based on this header.
#             # The Audit Group UI (ui_audit_group.py) will be responsible for constructing
#             # each row in `values_to_append` in this specific order.
#             # Example for data row construction in ui_audit_group.py:
#             # row_data = [
#             #     data.get('audit_group_number'),
#             #     calculate_audit_circle(data.get('audit_group_number')), # New
#             #     data.get('gstin'),
#             #     data.get('trade_name'),
#             #     data.get('category'),
#             #     data.get('total_amount_detected_overall_rs'),
#             #     data.get('total_amount_recovered_overall_rs'),
#             #     para.get('audit_para_number'),
#             #     para.get('audit_para_heading'),
#             #     para.get('revenue_involved_lakhs_rs'),
#             #     para.get('revenue_recovered_lakhs_rs'),
#             #     para.get('status_of_para'), # New
...
#         append_result = sheets_service.spreadsheets().values().append(
#             spreadsheetId=spreadsheet_id,
#             range=f"{first_sheet_title}!A1", # Appends after the last row with data
#             valueInputOption='USER_ENTERED',
#             body=body
#         ).execute()
#         return append_result
...
#             return pd.DataFrame()
#         else:
#             # Define the expected columns in the order they should appear in the DataFrame
#             expected_cols = [
#                 "Audit Group Number", "Audit Circle Number", "GSTIN", "Trade Name", "Category",
#                 "Total Amount Detected (Overall Rs)", "Total Amount Recovered (Overall Rs)",
#                 "Audit Para Number", "Audit Para Heading",
...
#                 "DAR PDF URL", "Record Created Date"
#             ]
#             # Check if the first row in 'values' looks like a header
#             if values and values[0] == expected_cols: # Ideal case: Header matches
#                 return pd.DataFrame(values[1:], columns=values[0])
#             elif values: # Data exists, try to use the first row as header
#                  # Heuristic: if the first row from sheet has a reasonable number of columns
#                  # and doesn't seem to be all numeric (which might indicate data without header), use it.
#                 if len(values[0]) >= len(expected_cols) - 3 and not all(isinstance(c, (int, float)) for c in values[0]):
#                     # If headers in sheet don't perfectly match expected_cols but seem valid, use them.
#                     # This provides flexibility if sheet was manually edited.
#                     df = pd.DataFrame(values[1:], columns=values[0])
#                     # Ensure all expected_cols are present, add if missing with NaN
#                     for col in expected_cols:
#                         if col not in df.columns:
#                             df[col] = pd.NA # Or None, or np.nan
#                     return df[expected_cols] # Reorder/select columns to match expected_cols
#                 else: # Fallback: no header or unidentifiable header
#                     return pd.DataFrame(values, columns=[f"Col{i+1}" for i in range(len(values[0]))]) # Generic column names

#             else: # Should be covered by 'if not values:'
#                 return pd.DataFrame()
...
# def delete_spreadsheet_rows(sheets_service, spreadsheet_id, sheet_id_gid, row_indices_to_delete):
#     # row_indices_to_delete are 0-based indices of the *data* rows (DataFrame iloc)
#     # Sheet rows are 1-based. Header is row 1. So data row 0 is sheet row 2.
#     if not row_indices_to_delete:
#         return True
#     requests = []
#     # Sort in descending order to avoid index shifting issues during deletion
#     for data_row_index in sorted(row_indices_to_delete, reverse=True):
#         sheet_row_start_index = data_row_index + 1 # Convert 0-based data index to 1-based sheet row index (data starts at row 2)
#         requests.append({
#             "deleteDimension": {
#                 "range": {
#                     "sheetId": sheet_id_gid,
#                     "dimension": "ROWS",
#                     "startIndex": sheet_row_start_index, # This is the 1-based index of the row in the sheet
#                     "endIndex": sheet_row_start_index + 1
#                 }
#             }
#         })
